...
from .networks.transformer_networks.rand_transformer import RandTransformer
from .networks.pvqvae_networks.auto_encoder import PVQVAE
...
class RandTransformerModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        import bpy
        tf_conf = OmegaConf.load(os.path.join(bpy.utils.resource_path('USER'), 'scripts/addons/AutoSDF_addon/AutoSDF', opt.tf_cfg))
        vq_conf = OmegaConf.load(os.path.join(bpy.utils.resource_path('USER'), 'scripts/addons/AutoSDF_addon/AutoSDF', opt.vq_cfg))
    ...
    def load_vqvae(self, vq_ckpt):
        ...
        import bpy
        state_dict = torch.load(os.path.join(bpy.utils.resource_path('USER'), 'scripts/addons/AutoSDF_addon/AutoSDF', vq_ckpt))
    ...
    def load_ckpt(self, ckpt):
        if type(ckpt) == str:
            import bpy
            state_dict = torch.load(os.path.join(bpy.utils.resource_path('USER'), 'scripts/addons/AutoSDF_addon/AutoSDF', ckpt))
        else:
            state_dict = ckpt
